...
export async function POST(nextRequest: NextRequest) {
  try {
    ...
    if (encryptedJwt !== undefined) {
      ...
      logger.info(`User: ${payload.id} Message: StoredPassword deleted successfully.`);
      return NextResponse.json({ message: "Successful!" }, { status: 200 });
    }

    logger.info(`Action: DeletePassword Message: No JWT Token. Password delete not successful`);
