...
export async function POST(nextRequest: NextRequest) {
  try {
    ...
    if (encryptedJwt !== undefined) {
      ...
      logger.info(
        `User ID: ${payload.id} Action: StoreNotes Message: Notes Stored successful`
      );
      return NextResponse.json({ message: "Successful!" }, { status: 200 });
    }

    logger.info(
      `Action :StoreNote Message: No JWT Token. Store Note not successful`
    );
