    ...
    if (encryptedJwt !== undefined) {
      ...
      await prisma.note.update({
        ...
        data: {
          ...
          encryptedContent: EncryptAES(
            vaultUpdateNoteData.content,
      ...
      logger.info(
        `User ID: ${payload.id} Action: UpdateStoredNote Message: Stored Note Updated successfully`
      );
      return NextResponse.json({ message: "Successful!" }, { status: 200 });
    }

    logger.info(
      `Action :UpdateNote Message: No JWT Token. Update Note not successful`
    );
