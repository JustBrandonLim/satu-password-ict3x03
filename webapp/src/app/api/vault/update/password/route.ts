...
export async function POST(nextRequest: NextRequest) {
  try {
    ...
    if (encryptedJwt !== undefined) {
      const vaultUpdatePasswordData: VaultUpdatePasswordData =
      ...
      await prisma.password.update({
        ...
        data: {
          ...
          encryptedPassword: EncryptAES(
            vaultUpdatePasswordData.password,
      ...
      logger.info(
        `User ID: ${payload.id} Action: UpdateStoredPassword Message: StoredPassword Updated successfully`
      );
      return NextResponse.json({ message: "Successful!" }, { status: 200 });
    }

    logger.info(
      `Action :UpdatePassword Message: No JWT Token. Update Password not successful`
    );
